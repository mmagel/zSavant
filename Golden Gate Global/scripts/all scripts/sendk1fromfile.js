function print_wires(recType,recId){
	...
	nlapiLogExecution('ERROR','s',source);
	var k1cover='';
	...
	if (fields.custentity236=='T' ){
		...
		if (source==7){
			...
			nlapiLogExecution('ERROR','ss','customsearch451');
			k1s=nlapiSearchRecord(null,'customsearch451');
		}
		else if (source==2){
			k1s=nlapiSearchRecord(null,'customsearch363');
		}
		else if (source==3){
			k1s=nlapiSearchRecord(null,'customsearch450');
		}
      else if (source==6){
			k1s=nlapiSearchRecord(null,'customsearch444');
		}
		var k1found=false;
		iid=iid.split('-');
		iid=iid[1]+'-'+iid[2];
		...
		if (k1found){
			...
			//['bwu@3gfund.com','ir@3gfund.com']
			nlapiSendEmail(7453,email,subject,body,['bwu@3gfund.com','ir@3gfund.com'],null,{'entity':recId},[k1,k1cover]);
