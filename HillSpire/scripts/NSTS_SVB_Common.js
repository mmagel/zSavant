    ...
    try
    {
           ...
           type=='void')
        {
            ...
            if (arrAllocJE)
            {
                ...
                var arrAllocJrnl = (type=='delete' || type=='void') ? arrAllocJE : recBill.getFieldValues('custbody_svb_allocation_journal');                
            ...
            if(type=='delete' || 
               type=='cancel')
            {                
                nlapiLogExecution('DEBUG','mainSVBProcess - Operation Delete/Cancel','******************************FINISHED********************************');
                return;
            }
            
            if(type=='void')
            {
                nlapiSubmitField('vendorbill',stBillId,'custbody_svb_vb_to_bell_dist_sched','');
                nlapiLogExecution('DEBUG','mainSVBProcess - Operation Void','******************************FINISHED********************************');
                return;
            }
...
function processBillDistribution(recBill,subtab,arrDSL)
{
    try
    {
        ...
        nlapiLogExecution('DEBUG','processBillDistribution | ' + subtab,'intCount=' + intCount);
...
function suiteletToCustomRecordMapping()
{    
    var arrFields = [];
        arrFields ['custrecord_svb_line_subsidiary']           = 'custpage_svb_details_subsidiary';
        arrFields ['custrecord_svb_line_department']           = 'custpage_svb_details_department';        
        arrFields ['custrecord_svb_line_location']             = 'custpage_svb_details_location';
        arrFields ['custrecord_svb_line_class']                = 'custpage_svb_details_class';
        arrFields ['custrecord_svb_line_intercomp_ap_account'] = 'custpage_svb_details_interco_ap_acct';
        arrFields ['custrecord_svb_line_intercomp_account']    = 'custpage_svb_details_interco_ar_acct';
        arrFields ['custrecord_svb_line_account']              = 'custpage_svb_details_account';
        arrFields ['custrecord_svb_line_intercomp_customer']   = 'custpage_svb_details_intercompcustomer';
        arrFields ['custrecord_svb_line_intercomp_vendor']     = 'custpage_svb_details_intercomp_vendor';        
...
/*
* //this function creates the journal entry allocation
*
* @version 1.1 MBuenavides - ICJE should not be automatically approved
*/
...
function createAllocJournalEntry(recBill,IS_INTERCO,stTrigFrom)
{
    ...
    if(IS_INTERCO!='T')
    {
        ...
        try
        {
			...
			var vendor=recBill.getFieldText('entity');
			var billrefnum=recBill.getFieldValue('tranid');
			recJE.setFieldValue('memo',vendor+' : '+billrefnum);
    ...
    else
    {
        ...
        for(var i=0; i<arrUniqToSubs.length; i++)
        {
            ...
            // MB : ICJE should not be automatically approved
            recICJE.setFieldValue('approved','F');
            ...
            for(line in arrSortedBDD)
            {
                ...
                {                                
                    ...
                    nlapiLogExecution('DEBUG', 'Create ICJE', 'setICJEParentLines');
                    recICJE = setICJEParentLines(recBill,recICJE,arrParentLine,stFrmSubs,objToDetails);                
                    nlapiLogExecution('DEBUG', 'Create ICJE', 'setICJEToSubsidiaryLines');
            ...
            try
            {
				...
				var vendor=recBill.getFieldText('entity');
				var billrefnum=recBill.getFieldValue('tranid');
				recICJE.setFieldValue('memo',vendor+' : '+billrefnum);
...
/*
* this function creates the "parent subsidiary" lines on the allocation journal entry
* @version 1.1 - MBuenavides: On the line, do not set eliminate to True 
*/
function setICJEParentLines(recBill,recICJE,arrSubsBDD,stFrmSubs,objToDetails)
{
	...
	var intLineNo    = arrSubsBDD.getValue('custrecord_svb_details_line_number');
	var intTab		 = arrSubsBDD.getValue('custrecord_svb_details_sublisttype')
	var memo;
	var vendor;
	var cust;
    nlapiLogExecution('DEBUG','intTab1',intTab);
    
    // get record type to determine if created from PO or not
    thetype = recBill.GetRecordType();
    if (thetype != 'purchaseorder')
    {
        
    }
    
			if (intTab==EXPENSE){
				memo=  recBill.getLineItemValue('expense','memo',intLineNo);
				stLineClass=recBill.getLineItemValue('expense','class',intLineNo);
				stLineDept=recBill.getLineItemValue('expense','department',intLineNo);
				stLineLoc=recBill.getLineItemValue('expense','location',intLineNo);
				cust=recBill.getLineItemValue('expense','custcol_invest_sub_15111',intLineNo);
            }
			else {
				memo=recBill.getLineItemValue('item','description',intLineNo);
				stLineClass=recBill.getLineItemValue('item','class',intLineNo);
				stLineDept=recBill.getLineItemValue('item','department',intLineNo);
				stLineLoc=recBill.getLineItemValue('item','location',intLineNo);
                cust=recBill.getLineItemValue('item','custcol_invest_sub_15111',intLineNo);
                //nlapiLogExecution('DEBUG','intTab==ITEMS here');
			}
			vendor=recBill.getFieldText('entity');
			nlapiLogExecution('DEBUG','memo1',memo+' '+intLineNo );
			if (memo!=''&&memo!=null&&memo.length>0){
				memo=vendor+' : '+memo;
			}
			else {
				memo=vendor;
    ...
    //recICJE.setLineItemValue('line','credit',currentLine,arrSubsBDD.getValue('custrecord_svb_details_amount'));
	var acctype=nlapiLookupField('account',stExpAccount,'type',true);
	var accnum=nlapiLookupField('account',stExpAccount,'number');
	
	
	if (acctype!='Other Income'&&acctype!='Other Expense'&&acctype!='Income'&&acctype!='Expense'&&acctype!='Fixed Asset'&&accnum.substring(0,3)!='116'&&accnum.substring(0,2)!='17'){
		recICJE.setLineItemValue('line','department',currentLine,'');
		recICJE.setLineItemValue('line','class',currentLine,'');
		recICJE.setLineItemValue('line','location',currentLine,'');
	}
	else {
    ...
    recICJE.setLineItemValue('line', 'memo', currentLine, memo);
	...
	if (stExpAccount==1324){
		nlapiLogExecution('DEBUG','stexp cust',cust+' '+stExpAccount );
		recICJE.setLineItemValue('line','entity',currentLine,cust);
	}
    //recICJE.setLineItemValue('line','debit',currentLine,arrSubsBDD.getValue('custrecord_svb_details_amount'));
	var clas='';
    var acctype=nlapiLookupField('account',stARAcct,'type',true);
	var accnum=nlapiLookupField('account',stARAcct,'number');
	nlapiLogExecution('DEBUG','cust 2',cust+' '+stARAcct );
	...
	if (stARAcct==1324){
		nlapiLogExecution('DEBUG','11111111111',cust+' '+stExpAccount );
		recICJE.setLineItemValue('line','entity',currentLine,cust);
	}
    // MB :  Do not set to True
    // recICJE.setLineItemValue('line','eliminate',currentLine,'T');
				...
				var proj=recICJE.getLineItemValue('line','entity',currentLine);
				if (proj!=null&&proj!=''){
					cat=nlapiLookupField('job',proj,'category');
				}
				if (cat==2){
					recICJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,nlapiLookupField('job',proj,'custentity_cseg_asset_class'));
				}
				else if (div==7){
					recICJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,ac);
				}
				else {
					recICJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,'');
				}
				
    nlapiLogExecution('DEBUG', 'setICJEParentLines', 'stExpAccount: ' + stExpAccount + ', stARAcct: ' + stARAcct
        + ', amount: ' + stToAmount);
    return recICJE;
}
...
function setICJEToSubsidiaryLines(recBill,recICJE,arrSubsBDD,stToSubs,objToDetails)
{
	...
	var memo;
	var vendor;
	nlapiLogExecution('DEBUG','intTab2',intTab);
	var cust;
	if (intTab==EXPENSE){
		memo= recBill.getLineItemValue('expense','memo',intLineNo);
		stLineClass=recBill.getLineItemValue('expense','class',intLineNo);
		...
		cust=recBill.getLineItemValue('item','custcol_invest_sub_15111',intLineNo);
			...
			vendor=recBill.getFieldText('entity');
			nlapiLogExecution('DEBUG','22222222222',memo+' '+intLineNo+' '+cust+' '+stAPAcct );
			if (memo!=''&&memo!=null&&memo.length>0){
				memo=vendor+' : '+memo;
			}
			else {
    ...
    recICJE.setLineItemValue('line', 'credit', currentLine, stToAmount);
    var acctype=nlapiLookupField('account',stAPAcct,'type',true);
	var accnum=nlapiLookupField('account',stAPAcct,'number');
	...
	if (stAPAcct==1324){
		nlapiLogExecution('DEBUG','bbbbbb',cust+' '+stAPAcct );
		recICJE.setLineItemValue('line','entity',currentLine,cust);
	}
    // MB: do not set to True
    // recICJE.setLineItemValue('line','eliminate',currentLine,'T');
    ...
    recICJE.setLineItemValue('line', 'debit', currentLine, stToAmount);
    var acctype=nlapiLookupField('account',stDestAcct,'type',true);
	var accnum=nlapiLookupField('account',stDestAcct,'number');
	...
	if (stDestAcct==1324){
		nlapiLogExecution('DEBUG','aaaaaaaa',cust+' '+stDestAcct );
		recICJE.setLineItemValue('line','entity',currentLine,cust);
	}
    recICJE.setLineItemValue('line', 'memo', currentLine, memo);
	var sub=stToSubs;
    ...
    nlapiLogExecution('DEBUG', 'setICJEToSubsidiaryLines', 'stAPAcct: ' + stAPAcct + ', stDestAcct: ' + stDestAcct + ', stToAmount: ' + stToAmount);
    return recICJE;
}
...
function setStandardJournalCreditLines(recBill,recJE)
{
    ...
    for(var x=0; x<arrSubTab.length; x++)
    {
        ...
        {            
			...
			var cust=recBill.getLineItemValue(arrSubTab[x],'custcol_invest_sub_15111',i);
            var memo;
			var vendor;
			if (arrSubTab[x]=='expense'){
				memo=  recBill.getLineItemValue(arrSubTab[x],'memo',i);
			}
			else {
				memo=recBill.getLineItemValue(arrSubTab[x],'description',i);
			}
			vendor=recBill.getFieldText('entity');
			if (memo!=''&&memo!=null&&memo.length>0){
				memo=vendor+' : '+memo;
            ...
            var acctype=nlapiLookupField('account',stBillLineAcct,'type',true);
			var accnum=nlapiLookupField('account',stBillLineAcct,'number');
			var clas='';
			
			if (acctype!='Other Income'&&acctype!='Other Expense'&&acctype!='Income'&&acctype!='Expense'&&acctype!='Fixed Asset'&&accnum.substring(0,3)!='116'&&accnum.substring(0,2)!='17'){
				recJE.setLineItemValue('line','department',currentLine,'');
				recJE.setLineItemValue('line','class',currentLine,'');
				recJE.setLineItemValue('line','location',currentLine,'');
			}
			else {
				...
				clas=stBillLineClass;
			}
			if (stBillLineAcct==1324){
				recJE.setLineItemValue('line','entity',currentLine,cust);
			}
            recJE.setLineItemValue('line','memo',currentLine,memo);
            ...
            var sub=recJE.getFieldValue('subsidiary');
				var div='';
				var ac=''
				var cat=''
				if (sub!=null&&sub!=''){
				...
				var proj=recJE.getLineItemValue('line','entity',currentLine);
				if (proj!=null&&proj!=''){
					cat=nlapiLookupField('job',proj,'category');
				}
				if (cat==2){
					recJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,nlapiLookupField('job',proj,'custentity_cseg_asset_class'));
				}
				else if (div==7){
					recJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,ac);
				}
				else {
					recJE.setLineItemValue('line','custcol_cseg_asset_class',currentLine,'');
				}
...
function setStandardJournalDebitLines(recBill,recJE,arrBDD)
{
    ...
    for(var i=0; i<arrBDD.length; i++)
    {
        ...
        nlapiLogExecution('DEBUG','setStandardJournalDebitLines','stDebitLineClass=' + stDebitLineClass + ' stDebitLineLoc=' + stDebitLineLoc + ' currentLine='     + currentLine);
		 ...
		 var cust;
			if (intTab==EXPENSE){
				memo=  recBill.getLineItemValue('expense','memo',intLineNo);
				stDebitLineClass=recBill.getLineItemValue('expense','class',i);
				stDebitLineDept=recBill.getLineItemValue('expense','department',i);
				stDebitLineLoc=recBill.getLineItemValue('expense','location',i);
				cust=recBill.getLineItemValue('expense','custcol_invest_sub_15111',i);
			}
			else {
				memo=recBill.getLineItemValue('item','description',intLineNo)
				stDebitLineClass=recBill.getLineItemValue('item','class',i);
				stDebitLineDept=recBill.getLineItemValue('item','department',i);
				stDebitLineLoc=recBill.getLineItemValue('item','location',i);
				cust=recBill.getLineItemValue('item','custcol_invest_sub_15111',i);
			}
			vendor=recBill.getFieldText('entity');
			if (memo!=''&&memo!=null&&memo.length>0){
				memo=vendor+' : '+memo;
		...
		recBill.getLineItemValue(intTab)
        arrDebitLine[intTab + '-' + intLineNo + '-' + fTotalLineAmt].push({account: stDebitLineAcct, debit: fDebitLineAmt, department: stDebitLineDept, classification: stDebitLineClass, location: stDebitLineLoc, memo:memo,cust:cust });
    }
    ...
    for(line in arrDebitLine)
    {
        ...
        for(var i=0; i<arrPerLine.length; i++)
        {
			...
			var acctype=nlapiLookupField('account',objJEDetails.account,'type',true);
			var accnum=nlapiLookupField('account',objJEDetails.account,'number');
			var clas='';
			...
			if (acctype!='Other Income'&&acctype!='Other Expense'&&acctype!='Income'&&acctype!='Expense'&&acctype!='Fixed Asset'&&accnum.substring(0,3)!='116'&&accnum.substring(0,2)!='17'){
				...
				clas=objJEDetails.classification;
			}
			if (objJEDetails.account==1324){
				recJE.setLineItemValue('line','entity',currentLine,objJEDetails.cust);
			}
          //  recJE.setLineItemValue('line','department',currentLine,objJEDetails.department);
           // recJE.setLineItemValue('line','class',currentLine,objJEDetails.classification);
           // recJE.setLineItemValue('line','location',currentLine,objJEDetails.location);
...
/*
* Conducts client-side checking, upon saving the bill
*
* @version 1.1 - MBuenavides 
* - Do not require Intercompany Vendor & Intercompany customer regardless of Intercompany AR/AP account
* - On the computation of allocation weight, handle up to 5 decimal places
*/
function onSaveBDSLValidations(type)
{
    ...
    var arrFlds = (isIntercoBillDistri=='T') ? INTERCO_MANDATORY_FLDS.trim().split(',') :
                                               NON_INTCO_MANDATORY_FLDS.trim().split(',');
    ...
    else
    {
        ...
        if(arrResult)
        {
            ...
            // MB: handle up to 5 decimal places only
            fTotalWt = parseFloat(fTotalWt).toFixed(5);
        }
