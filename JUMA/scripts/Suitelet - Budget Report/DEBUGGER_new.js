...
//lodash end
        ...
        var displayfy = "FY2018"; //request.getParameter('custpage_selectfield' );      // FY2018
        var displaytype = "YTDsum"; // request.getParameter('custpage_selectdisplay' );  // monthly
        var lastmonth = 1 ; //parseFloat( request.getParameter('custpage_selectmonth' ) );  // last month
        ...
        try{
            ...
            do {
                ...
                // Your code goes here to work on a the current resultSet (upto 1000 records per pass)
                    ...
                    amount = budgetsearch[i].getValue(cols[2]),
            ...
            nlapiLogExecution('DEBUG','uniquematches',JSON.stringify(uniquematches) );
            ...
            nlapiLogExecution('DEBUG','finalresult',JSON.stringify(finalresult) );
...
var stoppherenow=1;
                    ...
                    results.push({
                        "account" :   transonly[i].account , 
                        "tranamount" :   transonly[i].combinedtran, 
                        "a_chunkbudget" :   transonly[i].achunkbudget,                                  //YTD Budget Amount
                        "variance" :   transonly[i].combinedtran - transonly[i].achunkbudget,      //YTD Budget Variance
                        "b_chunkbudget" :   transonly[i].bchunkbudget,                                  //Remaining FY Budget
                        "projectedbudget" :   transonly[i].bchunkbudget + transonly[i].combinedtran       //Projected EOY Budget
                    });
                ...
                //  _.assign(a,b)  =  adds&overwrites on a with b
                // _.pick(a, ['amount','month'])
                // _.keys or _.values(a)
                // _.has (a, 'month')
                // _.find(a, function(month)
                //              return a.name )
                //
                // 

                //_.sortBy(finalresult, ['account'])
                sumamount = 0;
                sumbudget = 0;
                sumdifference = 0;
                newarray = [];

                var fg = _.groupBy(finalresult, 'account');
                var eachfg = _.map(fg, fg.account);

                for(var i=0; i<eachfg.length; i++){
                    for(var z=0; z<eachfg[i].length; z++){
                        sumamount = sumamount + parseFloat( eachfg[i][z].amount ) ;
...
/*
                    ...
                    // GROUP accounts
                    // SUM the amounts
                    var summedresult = results.reduce(function(acc, x) {
                        var id = acc[x.account]
                        if (id) {
                          //id.account = id.account
                          id.a_chunkbudget += x.a_chunkbudget
                          id.b_chunkbudget += x.b_chunkbudget
                          id.projectedbudget += x.projectedbudget
                          id.tranamount += x.tranamount
                          id.variance += x.variance
                        } else {
                          acc[x.account] = x
                          //delete x.account
                        }
