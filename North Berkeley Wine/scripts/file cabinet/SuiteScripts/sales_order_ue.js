...
/*
//set MLK B.O. and Retail O.R. on viewing SO - rbender @ 9/6/2017
	if ((type=='view')&&nlapiGetRole()==3){
		var so=nlapiLoadRecord('salesorder',nlapiGetRecordId());
		var form_no=so.getFieldValue('customform');
		...
		if (form_no==105){
            ...
            for (i=1; i-1<lines; i++){
                var internalID = so.getLineItemValue('item', 'item', i);
                ...
                so.setLineItemValue ( 'item' , 'custcol29' , i , qtyOnOrder );
            }
            ...
            for (i=1; i-1<lines; i++){
                ...
                so.setLineItemValue ( 'item' , 'custcol30' , i , qtyBackOrdered );
            }
		}
	}
// end setting field - rbender
*/
